...
# -----------------------------------------------------------------------------
# TIME OPERATION DETECTION PROMPT
# -----------------------------------------------------------------------------
def get_time_operation_detection_prompt(current_date):
    return f"""You determine what type of time-related operation is being requested. Today's date is {current_date}.
    
    Analyze the message to identify the operation type:
    1. TIME: Basic time queries - includes:
       - Current time: "what time is it", "what's the time", "current time"
       - Location time: "time in [location]", "what time is it in [location]"
       - Time differences: "time difference between [location] and [location]"
    2. REMINDER: Setting/managing reminders (handled separately)
    3. LOCATION: Updating timezone (handled separately)
    
    Consider conversation context and variations in phrasing.
    
    Respond with one of:
    - 'time': For time-related queries
    - 'none': For non-time messages
    """

# -----------------------------------------------------------------------------
# TIME RESPONSE FORMATTING PROMPT
# -----------------------------------------------------------------------------
def get_time_response_prompt():
    return """Format time-related responses naturally and clearly.
    
    Examples:
    - "It's 3:15 PM on Monday, January 15 in your timezone (America/New_York)"
    - "It's 4:15 AM on Tuesday, January 16 in Tokyo (Asia/Tokyo)"
    - "When it's 3:15 PM in New York, it's 8:15 PM in London. London is 5 hours ahead of New York."
    
    Include:
    1. Current time in 12-hour format with AM/PM
    2. Current date when relevant
